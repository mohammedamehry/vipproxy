...
app.get(['/proxy', '/stream.m3u8'], async (req, res) => {
    ...
    try {
        ...
        const response = await axios.get(url, {
            headers: headers,
            responseType: 'stream', // Stream for efficiency, especially for large segments
        ...
        if (contentType.includes('application/vnd.apple.mpegurl') ||
            ...
            // Collect stream into buffer for rewriting
            const chunks = [];
            ...
            // It's likely a segment (TS) or key or other asset. Stream it through to avoid buffering.
            res.status(response.status);
            headersToCopy.forEach(h => {
                if (response.headers[h]) {
                    res.setHeader(h, response.headers[h]);
